...
myProducts.map(mappingFunction);

async function mappingFunction(mappedProducts) {
  (productss.title = mappedProducts.title),
    (productss.description = mappedProducts.description),
    (productss.price = `${mappedProducts.price}`),
    (productss.discountPercentage = `${mappedProducts.discountPercentage}`),
    (productss.rating = `${mappedProducts.rating}`),
    (productss.stock = `${mappedProducts.stock}`),
    (productss.brand = mappedProducts.brand),
    (productss.category = mappedProducts.category),
    (productss.thumbnail = mappedProducts.thumbnail),
    (productss.images = mappedProducts.images);

  await axios
    .post("http://localhost:5000/api/v1/product/create", productss)
    .then((response) => {
